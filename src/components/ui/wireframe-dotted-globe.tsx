...
import type { Feature, FeatureCollection, MultiPolygon, Polygon, GeoJsonProperties } from "geojson"
...
export default function RotatingEarth({ width = 800, height = 600, className = "", markers = [], markerSize = 3.2 }: RotatingEarthProps) {
  ...
  useEffect(() => {
    ...
    const generateDotsInPolygon = (
      ...
      console.log(
        ...
        (feature.properties as Record<string, unknown> | null)?.featurecla || "Land",
      )
      return dots
    }
    ...
    const render = () => {
      ...
      if (landFeatures) {
        ...
        // Draw land outlines
        context.beginPath()
        landFeatures.features.forEach((feature: Feature<Polygon | MultiPolygon, GeoJsonProperties>) => {
        ...
        // Draw custom markers (bright white with subtle halo and stroke)
        if (Array.isArray(markers) && markers.length > 0) {
          markers.forEach((m) => {
            ...
            if (
              ...
              const r = markerSize * scaleFactor
              // Halo
              context.beginPath()
              context.arc(projected[0], projected[1], r * 1.75, 0, 2 * Math.PI)
              ...
              context.fill()
              // Stroke for contrast
              context.lineWidth = Math.max(1, 1.25 * scaleFactor)
    ...
    const loadWorldData = async () => {
      try {
        ...
        landFeatures = (await response.json()) as FeatureCollection<Polygon | MultiPolygon, GeoJsonProperties>
  ...
  }, [width, height, markers, markerSize])
